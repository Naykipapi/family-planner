...
const styles = `
  *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
  ...
  .budget-label{font-size:13px;color:var(--text-secondary);font-weight:500;display:flex;align-items:center;gap:6px}
  .budget-label button{background:none;border:none;cursor:pointer;font-size:14px;color:var(--text-tertiary);padding:0}
...
// FINANCES TAB
// ============================================
function FinancesTab({user,expenses,loading,onAddExpense,onUpdateExpense,onDeleteExpense,onRefresh,settings}){
  const[viewMode,setViewMode]=useState('individual');const[showForm,setShowForm]=useState(false);const[editingExpense,setEditingExpense]=useState(null);const[categoryFilter,setCategoryFilter]=useState(null);const[showCycleSettings,setShowCycleSettings]=useState(false);
  useEffect(()=>{onRefresh()},[onRefresh]);
  const now=new Date();
  const{start:cycleStart,end:cycleEnd}=getCycleDates(settings.cycleDay,now,settings.overrideDate);
  const cycleExpenses=expenses.filter(e=>{const d=new Date(e.date);return d>=cycleStart&&d<=cycleEnd});
  const filtered=viewMode==='individual'?cycleExpenses.filter(e=>e.who===user.name):cycleExpenses;
  ...
  const byCategory={};filtered.forEach(e=>{byCategory[e.category]=(byCategory[e.category]||0)+e.amount});
  ...
  return(<>
    ...
    <div className="budget-card">
      <div className="budget-header"><span className="budget-label">Budget <button onClick={()=>setShowCycleSettings(!showCycleSettings)}>⚙️</button></span><span className="budget-amount">{formatCurrency(remaining)}</span></div>
      ...
      <div className="budget-period">{formatPeriod(cycleStart,cycleEnd)}</div>
    </div>
    ...
    {showExpenseList&&<div className="section"><div className="section-title">Expenses</div>{showForm?<ExpenseForm user={user} onSave={onAddExpense} onCancel={()=>setShowForm(false)} saveLabel="Add Expense"/>:<button className="add-form-toggle" onClick={()=>setShowForm(true)}><span style={{fontSize:16,lineHeight:1}}>+</span> Add Expense</button>}{categoryFilter&&<div className="filter-pill">{CATEGORY_ICONS[categoryFilter]||'•'} {categoryFilter}<button onClick={()=>setCategoryFilter(null)}>✕</button></div>}{loading?<div className="loading">Loading expenses...</div>:recent.length===0?<div className="empty-state">{categoryFilter?'No expenses in this category.':'No expenses this period yet.'}</div>:recent.map((e,i)=>(<div className="expense-item" key={i} onClick={()=>setEditingExpense(e)}><div className="expense-icon">{CATEGORY_ICONS[e.category]||'•'}</div><div className="expense-details"><div className="expense-merchant">{e.merchant}</div><div className="expense-meta">{formatDate(e.date)} · {e.category}{e.account?` · ${e.account}`:''}</div></div><div className="expense-amount">{formatCurrency(e.amount)}</div></div>))}</div>}
    {!showExpenseList&&<div className="section">{showForm?<ExpenseForm user={user} onSave={onAddExpense} onCancel={()=>setShowForm(false)} saveLabel="Add Expense"/>:<button className="add-form-toggle" onClick={()=>setShowForm(true)}><span style={{fontSize:16,lineHeight:1}}>+</span> Add Expense</button>}</div>}
    {monthHistory.length>1&&<div className="section"><div className="section-title">Period History</div>{monthHistory.map((m,i)=>(<div className="month-history-row" key={i}><span className="month-history-label">{m.label}</span><span className="month-history-amount">{formatCurrency(m.total)}</span></div>))}</div>}
