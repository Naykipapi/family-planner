...
const styles = `
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  :root {
    ...
    --accent: #1a1a1a;
    --green: #2d8a4e; --green-bg: #e8f5e9;
    --amber: #c17f24; --amber-bg: #fff3e0;
  ...
  .tab-btn { flex: 1; max-width: 200px; display: flex; flex-direction: column; align-items: center; gap: 2px; padding: 6px 0; background: none; border: none; cursor: pointer; font-family: var(--font-body); font-size: 10px; font-weight: 500; color: var(--text-tertiary); transition: color 0.15s; }
  ...
  .view-toggle, .cal-view-toggle { display: flex; margin: 0 16px 16px; background: var(--bg-input); border-radius: var(--radius-sm); padding: 3px; gap: 2px; }
  .view-toggle button, .cal-view-toggle button { flex: 1; padding: 7px 0; border: none; background: transparent; border-radius: 5px; font-family: var(--font-body); font-size: 13px; font-weight: 500; color: var(--text-secondary); cursor: pointer; transition: all 0.15s; }
  .view-toggle button.active, .cal-view-toggle button.active { background: var(--bg-card); color: var(--text-primary); box-shadow: var(--shadow); }
  .cal-view-toggle { margin-bottom: 12px; }
  .cal-view-toggle button { font-size: 12px; padding: 6px 0; }
  ...
  .expense-merchant { font-size: 14px; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .expense-meta { font-size: 12px; color: var(--text-tertiary); margin-top: 1px; }
  .expense-amount { font-size: 14px; font-weight: 600; font-variant-numeric: tabular-nums; flex-shrink: 0; }
  ...
  .week-day-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-tertiary); margin-bottom: 6px; }
  ...
  .confirm-dialog { padding: 20px; text-align: center; }
  ...
  .pending-row { display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid var(--border); font-size: 14px; }
  ...
  .pending-result { text-align: center; padding: 24px; font-size: 15px; }
...
const CATEGORY_ICONS = {
  'Food & Drink': '🍽', 'Shopping': '🛍', 'Activities & Entertainment': '🎭',
  'Travel': '✈️', 'Sports & Wellness': '🏋️', 'Health & Medical': '🩺',
  // Legacy mapping
  'Health & Wellness': '🏋️',
};
...
function getEventWho(event) {
  ...
  const raw = match[1];
  // Match partial names: "Lena-Marie" -> "Lena", "Telman" -> "Telman"
  for (const u of CONFIG.USERS) {
    if (raw.toLowerCase().startsWith(u.toLowerCase())) return u;
  }
...
function matchUserName(googleName) {
  const first = (googleName || '').split(' ')[0];
  for (const u of CONFIG.USERS) {
    if (first.toLowerCase().startsWith(u.toLowerCase()) || u.toLowerCase().startsWith(first.toLowerCase())) return u;
  }
  return first;
}

function getEventsForDate(calEvents, date) {
...
function useGoogleAuth() {
  ...
  useEffect(() => {
    ...
    const loadGsi = () => {
      if (window.google?.accounts?.oauth2) {
        ...
        setTokenClient(client); setGsiReady(true);
      } else setTimeout(loadGsi, 100);
    };
    loadGapi(); loadGsi();
  }, []);
  ...
  const fetchUserInfo = async (accessToken) => {
    try {
      ...
      const name = matchUserName(info.given_name || info.name);
      setUser({ name, email: info.email, picture: info.picture });
...
function useSheets() {
  ...
  const fetchExpenses = useCallback(async () => {
    setLoading(true);
    try {
      const response = await window.gapi.client.sheets.spreadsheets.values.get({ spreadsheetId: CONFIG.SPREADSHEET_ID, range: 'Expenses!A2:H10000' });
      const rows = response.result.values || [];
      setExpenses(rows.map((r, i) => ({ rowIndex: i + 2, merchant: r[0] || '', amount: parseFloat(r[1]) || 0, date: r[2] || '', category: r[3] || '', who: r[4] || '', tag: r[5] || '', notes: r[6] || '', account: r[7] || '' })));
  ...
  const addExpense = useCallback(async (expense) => {
    try {
      await window.gapi.client.sheets.spreadsheets.values.append({ spreadsheetId: CONFIG.SPREADSHEET_ID, range: 'Expenses!A:H', valueInputOption: 'USER_ENTERED',
      ...
      await fetchExpenses(); return true;
    } catch (err) { console.error('Sheets append error:', err); return false; }
  }, [fetchExpenses]);

  const updateExpense = useCallback(async (rowIndex, expense) => {
    try {
      await window.gapi.client.sheets.spreadsheets.values.update({ spreadsheetId: CONFIG.SPREADSHEET_ID, range: `Expenses!A${rowIndex}:H${rowIndex}`, valueInputOption: 'USER_ENTERED',
        resource: { values: [[expense.merchant, expense.amount, expense.date, expense.category, expense.who, expense.tag || '', expense.notes || '', expense.account || '']] } });
      await fetchExpenses(); return true;
  ...
  const deleteExpense = useCallback(async (rowIndex) => {
    try {
      ...
      await window.gapi.client.sheets.spreadsheets.batchUpdate({ spreadsheetId: CONFIG.SPREADSHEET_ID,
...
function useCalendar() {
  ...
  const fetchEvents = useCallback(async (timeMin, timeMax) => {
    setLoading(true);
    try {
      const response = await window.gapi.client.calendar.events.list({ calendarId: CONFIG.CALENDAR_ID, timeMin: timeMin.toISOString(), timeMax: timeMax.toISOString(), singleEvents: true, orderBy: 'startTime', maxResults: 200 });
  ...
  const addEvent = useCallback(async (event) => {
    try {
      ...
      await window.gapi.client.calendar.events.insert({ calendarId: CONFIG.CALENDAR_ID, resource }); return true;
  ...
  const updateEvent = useCallback(async (eventId, event) => {
    try {
      ...
      await window.gapi.client.calendar.events.update({ calendarId: CONFIG.CALENDAR_ID, eventId, resource }); return true;
    } catch (err) { console.error('Calendar update error:', err); return false; }
  }, []);

  const deleteEvent = useCallback(async (eventId) => {
    try { await window.gapi.client.calendar.events.delete({ calendarId: CONFIG.CALENDAR_ID, eventId }); return true; }
  ...
  return (
    <div className="add-form">
      <div className="form-row">
        ...
        <div className="form-field" style={{flex:1}}><label>Amount €</label><input type="number" step="0.01" value={form.amount} onChange={e=>set('amount',e.target.value)} placeholder="0"/></div>
      </div>
      <div className="form-row">
...
function EventForm({ initial, user, onSave, onCancel, saveLabel }) {
  ...
  const [form, setForm] = useState({
    ...
    endDate: initial?.endDate||initial?.startDate||today, endTime: initial?.endTime||'10:00', allDay: initial?.allDay||false,
    location: initial?.location||'', who: initial?.who||user?.name||'Telman', category: initial?.category||CONFIG.EVENT_CATEGORIES[0],
  });
  const [endManuallySet, setEndManuallySet] = useState(!!initial);
  const [saving, setSaving] = useState(false);

  // Auto-sync end date to start date unless manually changed
  const setField = (k, v) => {
    setForm(f => {
      const next = { ...f, [k]: v };
      if (k === 'startDate' && !endManuallySet) next.endDate = v;
      if (k === 'endDate') setEndManuallySet(true);
      return next;
    });
  };
  ...
  return (
    <div className="add-form">
      <div className="form-row"><div className="form-field"><label>Title</label><input value={form.title} onChange={e=>setField('title',e.target.value)} placeholder="e.g. Dinner with friends"/></div></div>
      <div className="form-row">
        <div className="form-field"><label>Who</label><select value={form.who} onChange={e=>setField('who',e.target.value)}>{[...CONFIG.USERS,'Family'].map(u=><option key={u} value={u}>{u}</option>)}</select></div>
        <div className="form-field"><label>Category</label><select value={form.category} onChange={e=>setField('category',e.target.value)}>{CONFIG.EVENT_CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
      </div>
      <div className="form-row">
        <div className="form-field" style={{flex:'none'}}><label>All Day</label><input type="checkbox" checked={form.allDay} onChange={e=>setField('allDay',e.target.checked)} style={{width:18,height:18,marginTop:4}}/></div>
        <div className="form-field"><label>Start Date</label><input type="date" value={form.startDate} onChange={e=>setField('startDate',e.target.value)}/></div>
        {!form.allDay&&<div className="form-field"><label>Start Time</label><input type="time" value={form.startTime} onChange={e=>setField('startTime',e.target.value)}/></div>}
      </div>
      <div className="form-row">
        <div className="form-field"><label>End Date</label><input type="date" value={form.endDate} onChange={e=>setField('endDate',e.target.value)}/></div>
        {!form.allDay&&<div className="form-field"><label>End Time</label><input type="time" value={form.endTime} onChange={e=>setField('endTime',e.target.value)}/></div>}
      </div>
      <div className="form-row"><div className="form-field"><label>Location (optional)</label><input value={form.location} onChange={e=>setField('location',e.target.value)} placeholder="e.g. Restaurant Zum Löwen"/></div></div>
      <div className="form-actions">
        <button className="btn-secondary" onClick={onCancel}>Cancel</button>
...
function EditExpenseModal({ expense, user, onUpdate, onDelete, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}><div className="modal-content" onClick={e=>e.stopPropagation()}>
      ...
      <ExpenseForm initial={expense} user={user} onSave={async(u)=>{const s=await onUpdate(expense.rowIndex,u);if(s)onClose();return s;}} onCancel={onClose} saveLabel="Save Changes"/>
...
function EditEventModal({ event, user, onUpdate, onDelete, onClose, onRefresh }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}><div className="modal-content" onClick={e=>e.stopPropagation()}>
      ...
      <EventForm initial={initial} user={user} onSave={async(u)=>{const s=await onUpdate(event.id,u);if(s){onRefresh();onClose();}return s;}} onCancel={onClose} saveLabel="Save Changes"/>
...
function FinancesTab({ user, expenses, loading, onAddExpense, onUpdateExpense, onDeleteExpense, onRefresh }) {
  ...
  // #14: In individual view, show expenses where who matches user
  ...
  // #15: In combined view, don't show individual expenses — only categories + budget
  ...
  return (
    <>
      <div className="view-toggle">
        <button className={viewMode==='individual'?'active':''} onClick={()=>{setViewMode('individual');setCategoryFilter(null);}}>{user.name}</button>
        <button className={viewMode==='combined'?'active':''} onClick={()=>{setViewMode('combined');setCategoryFilter(null);}}>Combined</button>
      </div>
      ...
      {categoryList.length > 0 && (
        <div className="section">
          ...
          {categoryList.map(([cat, amount]) => (
            <div className={`cat-row ${categoryFilter===cat?'active':''}`} key={cat} onClick={()=>showExpenseList && setCategoryFilter(categoryFilter===cat?null:cat)}>
      ...
      {/* Combined view: still allow adding expenses */}
      {!showExpenseList && (
        <div className="section">
          {showForm ? <ExpenseForm user={user} onSave={onAddExpense} onCancel={()=>setShowForm(false)} saveLabel="Add Expense"/> : (
            <button className="add-form-toggle" onClick={()=>setShowForm(true)}><span style={{fontSize:16,lineHeight:1}}>+</span> Add Expense</button>
...
function CalendarTab({ user, events: calEvents, loading, onFetchEvents, onAddEvent, onUpdateEvent, onDeleteEvent }) {
  ...
  const [upcomingMode, setUpcomingMode] = useState('month'); // month | all
  ...
  // #17 + #18: Upcoming events - only future, with toggle
  const nowTime = today.getTime();
  const futureEvents = calEvents.filter(ev => {
    const start = new Date(ev.start?.dateTime || ev.start?.date);
    const end = new Date(ev.end?.dateTime || ev.end?.date);
    return end.getTime() >= nowTime;
  });

  const upcomingEvents = upcomingMode === 'month'
    ? futureEvents.filter(ev => {
        const start = new Date(ev.start?.dateTime || ev.start?.date);
    ...
    <>
      ...
      {calView==='month' && !loading && (
        <div className="month-grid">
          ...
          <div className="month-grid-body">
            {cells.map((cell,i) => {
              const dayEvents = getEventsForDate(calEvents, cell.date);
              return (
                <div key={i} className={`month-day ${cell.otherMonth?'other-month':''} ${cell.isToday?'today':''}`}>
                  <div className="day-number">{cell.day}</div>
                  {dayEvents.slice(0,3).map((ev,j)=>{const who=getEventWho(ev);return <div key={j} className={`day-event ${getWhoClass(who)}`} onClick={()=>setEditingEvent(ev)}>{cleanEventTitle(ev)}</div>;})}
      ...
      {calView==='week' && !loading && (
        <div className="week-view">
          {weekDays.map((day,i) => {
            const dayEvents = getEventsForDate(calEvents, day);
            ...
            return (
              <div className="week-day-card" key={i}>
                ...
                {dayEvents.length===0?<div className="week-no-events">No events</div>:dayEvents.map((ev,j)=>{
                  ...
                  let timeStr = 'All day';
                  if(startDt&&endDt){const s=startDt.toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'});const e2=endDt.toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'});timeStr=`${s}–${e2}`;}
      ...
      {/* #18: Upcoming with toggle - only in month view */}
      {calView==='month' && (
        <div className="section">
          <div className="upcoming-toggle">
            <button className={upcomingMode==='month'?'active':''} onClick={()=>setUpcomingMode('month')}>This Month</button>
            <button className={upcomingMode==='all'?'active':''} onClick={()=>setUpcomingMode('all')}>All Upcoming</button>
          </div>
          {upcomingEvents.length===0&&!loading?<div className="empty-state">No upcoming events.</div>:upcomingEvents.slice(0,15).map((ev,i)=>{
            const who = getEventWho(ev);
      ...
      {/* #19: Week view - only upcoming events (future from today) */}
      {calView==='week' && (
        <div className="section">
          <div className="section-title">Upcoming</div>
          {futureEvents.length===0&&!loading?<div className="empty-state">No upcoming events.</div>:futureEvents.slice(0,10).map((ev,i)=>{
            const who = getEventWho(ev);
            const start = ev.start?.dateTime?new Date(ev.start.dateTime):new Date(ev.start?.date);
            const timeStr = ev.start?.dateTime
              ? start.toLocaleDateString('de-DE',{day:'numeric',month:'short'})+' · '+start.toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'})
    ...
    try {
      if (isExpense) {
        ...
        for (const exp of expenses) {
          const success = await onAddExpense({ merchant:exp.merchant||'', amount:parseFloat(exp.amount)||0, date:exp.date||new Date().toISOString().split('T')[0], category:exp.category||'', who:exp.who||'Telman', tag:exp.tag||'', notes:exp.notes||'', account:exp.account||'' });
          if (!success) allOk = false;
        }
        if (allOk) { await fetchExpenses(); setStatus('success'); setResultMsg(`${expenses.length} expense(s) added`); }
        else { setStatus('error'); setResultMsg('Some expenses failed to save'); }
      } else if (isEvent) {
        const eventData = { title:data.title||'', who:data.who||'Telman', category:data.category||'Social', location:data.location||'', allDay:!!data.all_day };
        if (data.all_day) { eventData.startDate=data.start; eventData.endDate=data.end||data.start; }
        else { eventData.startDateTime=data.start; eventData.endDateTime=data.end||data.start; }
        const success = await onAddEvent(eventData);
        if (success) { setStatus('success'); setResultMsg(`Event "${data.title}" created`); }
        else { setStatus('error'); setResultMsg('Failed to create event'); }
      }
    } catch (err) { setStatus('error'); setResultMsg('Error: '+err.message); }
  };

  const handleDismiss = () => { window.location.hash=''; onDismiss(); };
  ...
  return (
    <div className="pending-screen">
      ...
      <div className="pending-subtitle">
        ...
        {status==='sending'&&'Saving...'}{status==='success'&&'Done!'}{status==='error'&&'Something went wrong'}
      </div>
      ...
      {status==='ready'&&isExpense&&!isBatch&&(
        <div className="pending-card">
          ...
          <div className="pending-row"><span className="pending-row-label">Amount</span><span className="pending-row-value">{formatCurrency(data.amount)}</span></div>
      ...
      {status==='ready'&&isExpense&&isBatch&&(<>{expenses.map((exp,i)=>(
        <div className="pending-batch-item" key={i}><div><div className="pending-batch-merchant">{exp.merchant}</div><div className="pending-batch-meta">{exp.date} · {exp.category} · {exp.account}</div></div><div className="pending-batch-amount">{formatCurrency(exp.amount)}</div></div>
      ))}</>)}
...
function SignInScreen({ onSignIn, ready }) {
  return (
    <div className="sign-in-screen">
      <h1>Family Planner</h1><p>Sign in with Google to get started</p>
      <button className="google-btn" onClick={onSignIn} disabled={!ready}><GoogleIcon/> Sign in with Google</button>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    const checkHash = () => {
      ...
      if (hash.startsWith('#add/')) {
        try { const json = decodeURIComponent(atob(hash.slice(5))); setPendingAction(JSON.parse(json)); } catch(e){ console.error('Failed to parse pending action:',e); }
      }
    };
    checkHash();
  ...
  if (pendingAction && user) return (<><style>{styles}</style><PendingAction data={pendingAction} user={user}
    onAddExpense={async(e)=>{const s=await addExpense(e);return s;}} onAddEvent={async(e)=>{const s=await addEvent(e);return s;}}
    onDismiss={()=>setPendingAction(null)} fetchExpenses={fetchExpenses}/></>);

  if (pendingAction && !user) return (<><style>{styles}</style>
    <div className="sign-in-screen"><h1>Family Planner</h1><p>Sign in to confirm the pending action</p>
    <button className="google-btn" onClick={signIn} disabled={!ready}><GoogleIcon/> Sign in with Google</button></div></>);
